 ...
 * @todo confirm link to time and resume from playback work together
 * @todo refactor resume playback from cookie to wp user data via ajax
 */
...
(() => {
    ...
    class plugin {
        constructor(playerInstance) {
            ...
            let offset = hash.indexOf('t=') ? seconds(window.location.hash.substr(3) + '.000') : false;
            let shouldPlay = !!hash;
            ...
            let cookieData = Cookies.get(`jw_${playerInstance}_resumevideodata`);
            ...
            playerInstance.on('ready', () => {
                ...
                playerInstance.on('time', function (e) {
                    Cookies.set(`jw_${playerInstance}_resumevideodata`, `${Math.floor(e.position)}:${playerInstance.getDuration()}`);
                });
